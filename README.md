...
* 🐳 **容器友善**：支援 **Podman / Docker** 容器化構建與部署，提供 Rootless 與 SELinux `:Z` 支援。
...
* **容器化**：Podman / Docker (Alpine Linux 基礎映像檔，大小 < 25MB)
...
### 方法一：使用 Podman / Docker（Linux / 通用推薦）
...
### 方法二：Mac mini (Apple Silicon / M2) 原生部署

因為是純 Go 實作，可直接在 macOS 上原生編譯執行，免開虛擬機，記憶體佔用極低。
...
mkdir -p bin data
go build -ldflags="-s -w" -o bin/random_eat ./cmd/bot
```

#### 2. 使用 macOS `launchd` 設定開機自啟與後台常駐
建立服務檔 `~/Library/LaunchAgents/com.randomeat.bot.plist`：
```xml
...
<dict>
    <key>Label</key>
    <string>com.randomeat.bot</string>
    <key>ProgramArguments</key>
    <array>
        <string>/Users/你的使用者名稱/apps/random_eat_discord/bin/random_eat</string>
    </array>
    <key>WorkingDirectory</key>
    <string>/Users/你的使用者名稱/apps/random_eat_discord</string>
    <key>RunAtLoad</key>
    <true/>
    <key>KeepAlive</key>
    <true/>
    <key>StandardOutPath</key>
    <string>/Users/你的使用者名稱/apps/random_eat_discord/data/bot.log</string>
    <key>StandardErrorPath</key>
    <string>/Users/你的使用者名稱/apps/random_eat_discord/data/bot.err</string>
</dict>
</plist>
```
...
#### 3. 載入並啟動服務
```bash
launchctl load ~/Library/LaunchAgents/com.randomeat.bot.plist
```
